...
const sendWithResend = async (to, subject, html) => {
  return new Promise((resolve, reject) => {
    const data = JSON.stringify({
      from: `KAIZEN <${process.env.RESEND_FROM || 'onboarding@resend.dev'}>`,
      to: [to],
      subject,
      html
    });
...
const sendWelcomeEmail = async (toEmail, username) => {
  if (!process.env.RESEND_API_KEY) {
    console.log('No Resend key — skipping welcome email');
    return;
  }
  try {
    await sendWithResend(
      ...
      <head>
        <style>
          ...
          .wrap{max-width:560px;margin:0 auto;background:#111;border:1px solid rgba(201,168,76,0.2);}
          .head{padding:32px 36px;border-bottom:1px solid rgba(255,255,255,0.06);}
          .logo{font-size:1.2rem;color:#C9A84C;letter-spacing:0.1em;font-weight:700;font-family:monospace;}
          .body{padding:36px;}
          ...
          .foot{padding:20px 36px;border-top:1px solid rgba(255,255,255,0.06);}
      ...
      <body>
        <div class="wrap">
          ...
          <div class="body">
            ...
            <p class="text">Your discipline journey starts today. KAIZEN is not a trading signal app. It is a <strong>behavioral accountability system</strong> built to help you follow your own rules, consistently, every single session.</p>
            <p class="text">The traders who win long-term are not the smartest. They are the most <strong>disciplined</strong>.</p>
            <div class="box">
              <div class="box-label">Your Starting Discipline Score</div>
              <div class="box-val">0%</div>
              <p style="font-size:0.78rem;color:#A8A09A;margin-top:8px;margin-bottom:0;">Every compliant session raises this. Every violation drops it. Make it go up.</p>
            </div>
            <p class="text">Start by logging your first session in My Journal. Be honest. KAIZEN AI reads every word.</p>
            <a href="${process.env.APP_URL || 'https://kaizen-trader.onrender.com'}/dashboard" class="btn">Open Dashboard</a>
          </div>
          <div class="foot">
...
const sendOTPEmail = async (toEmail, otp) => {
  if (!process.env.RESEND_API_KEY) {
    console.log('No Resend key — OTP:', otp);
    return;
  }
  try {
    await sendWithResend(
      ...
      <head>
        <style>
          body{margin:0;padding:0;background:#080808;font-family:Arial,sans-serif;}
          .wrap{max-width:480px;margin:0 auto;background:#111;border:1px solid rgba(201,168,76,0.2);}
      ...
      <body>
        <div class="wrap">
          ...
          <div class="body">
            ...
            <div class="sub">Enter this 3-digit code to complete registration</div>
            <div class="otp-box"><div class="otp-code">${otp}</div></div>
            <p class="note">Expires in <strong style="color:#F5F3EF;">10 minutes</strong>.</p>
          </div>
          <div class="foot"><p class="foot-text">改善 — KAIZEN · Trade Better. Every Day.</p></div>
        </div>
      </body>
      </html>`
    );
